...
class JsonschemageneratorValidationException implements Exception {
  final List<String> errors;

  JsonschemageneratorValidationException(this.errors);

  @override
  String toString() => 'JsonschemageneratorValidationException: ${errors.join("; ")}';
}
...
/// JSON Schema Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/jsonschemagenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [json] (required) - The sample JSON data to generate a schema from
/// * [title] - The title for the generated schema (default: 'Generated Schema')
/// * [description] - Optional description for the schema
class JsonschemageneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'title': ValidationRule(type: 'string', required: false),
    'description': ValidationRule(type: 'string', required: false),
  };

  JsonschemageneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/jsonschemagenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [JsonschemageneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw JsonschemageneratorValidationException(errors);
    }
  }

  /// Execute a request to the JSON Schema Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<JsonschemageneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw JsonschemageneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return JsonschemageneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw JsonschemageneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw JsonschemageneratorException('Resource not found');
      } else {
        throw JsonschemageneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is JsonschemageneratorException) rethrow;
      throw JsonschemageneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the JSON Schema Generator API client.
class JsonschemageneratorException implements Exception {
  final String message;

  JsonschemageneratorException(this.message);

  @override
  String toString() => 'JsonschemageneratorException: $message';
}
